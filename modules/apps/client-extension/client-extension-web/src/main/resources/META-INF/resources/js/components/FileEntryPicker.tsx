...
const FileEntryPicker = ({
	...
	const handleFrontendTokenDefinitionFileValidation = async (file: File) => {
		handlePost(objectToFormData({file}), '/validate-file')
			.then(async (response) => {
				return {
			...
			.then(({data, isResponseOk}) => {
				...
				disableFormSubmitButton(!isResponseOk, portletNamespace);
			})
			.catch(() => {
	...
	const handleUserFile = async (file: File) => {
		let frontendTokenDefinitionString = await readInputFile(file);

		if (!frontendTokenDefinitionString) {
			file = new File(['{}'], 'frontendTokenDefinition.json', {
				type: 'application/json',
			});
			frontendTokenDefinitionString = '{}';
		}

		await handleFrontendTokenDefinitionFileValidation(file);

		setFrontendTokenDefinition(frontendTokenDefinitionString);
	...
	const handleFileInputChange = async ({
		...
		handleUserFile(target.files[0]);
	};

	const getFrontendTokenDefinitionInfo = () => {
		handlePost(
			objectToFormData({
	...
	useEffect(() => {
		if (frontendTokenDefinition && externalReferenceCode) {
			getFrontendTokenDefinitionInfo();
		}
